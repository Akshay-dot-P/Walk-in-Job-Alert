...
# Source-driven experience framing. Raw external resume text is never copied into
# generated resumes; we only extract high-level role vocabulary and bullet style.
#
# IMPORTANT: Do not use the repo's resumes/ folder as research input. Those are
# generated outputs from this script, so feeding them back into tailoring would
# create a model-on-model feedback loop.
...
def _build_source_driven_experience_fallback(experience_research: dict | None = None) -> dict:
    """
    Last-resort fallback for the work-experience section.
    It uses only the candidate's real current-role facts plus source-derived
    transition bridges. It intentionally avoids static domain/role buckets.
    """
...
def generate_amazon_bullets_dynamic(job: dict, jd_keywords: dict,
    ...
    research_context = format_experience_research_prompt(
        experience_research,
        for_work_experience=True,
    )
    kw_hint = (
...
- never claim tools, certifications, platforms, or source-resume achievements in work experience
...
#
# This replaces static "role bucket decides the experience section" behavior
# with a research summary built from public/user-provided resume PDFs/DOCX files
# and Reddit/forum discussions. Generated repo resumes are intentionally ignored.
# External text is only used to extract patterns; it is scrubbed and never copied
# into the resume.
# ─────────────────────────────────────────────────────────────────────────────
_RESEARCH_STOPWORDS = {
...
_RESUME_ACTION_VERBS = {
    "analyzed", "assessed", "audited", "automated", "built", "conducted",
    "configured", "created", "detected", "developed", "documented", "enforced",
    "evaluated", "identified", "implemented", "investigated", "maintained",
    "monitored", "performed", "prioritized", "reviewed", "scanned", "triaged",
    "validated",
}
...
_TECHNICAL_BENCHMARK_TERMS = {
    "splunk", "python", "bash", "powershell", "virustotal", "abuseipdb",
    "urlscan", "wireshark", "nmap", "aws", "boto3", "nessus", "openvas",
    "burp", "metasploit", "qradar", "sentinel", "crowdstrike", "defender",
    "elastic", "kibana", "sysmon", "guardduty", "cloudtrail", "cyberark",
    "sailpoint", "okta", "sql", "linux", "windows", "dns", "api", "siem",
    "osint", "threat intelligence", "ioc", "mitre", "owasp",
}
_WORK_EXPERIENCE_SAFE_SIGNALS = (
    "triage", "escalat", "incident", "investig", "document", "audit",
    "evidence", "root cause", "anomal", "pattern", "severity", "risk",
    "compliance", "policy", "control", "fraud", "transaction", "monitor",
    "case", "sla", "workflow", "handoff", "priority", "report",
)
...
def _summarize_experience_research(job: dict, jd_keywords: dict, resume_sources: list[dict], forum_sources: list[dict]) -> dict:
    ...
    experience_terms = [term for term in target_terms if _is_work_experience_safe_term(term)]
    technical_terms = [term for term in target_terms if term not in experience_terms]
    ...
    summary_parts.append("Use these as framing signals only; do not copy wording or borrow tools/certs from external resumes.")
...
def collect_experience_research(job: dict, jd_keywords: dict) -> dict:
    ...
    research = _summarize_experience_research(
        job,
        jd_keywords,
        public_sources,
        forum_sources,
...
def main():
    ...
    for i, job in enumerate(pending, 1):
        ...
        try:
            ...
            # Source-driven resume/forum benchmarks for experience framing
            logger.info("  Collecting resume/forum benchmarks...")
            experience_research = collect_experience_research(job, jd_keywords)
